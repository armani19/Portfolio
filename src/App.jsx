...
const interestCards = [
  { title: 'Currently Exploring', color: 'blue', items: ['Project Management', 'Frontend Development', 'UI / UX Specialist'] },
  { title: 'Design', color: 'pink', items: ['Figma', 'Canva', 'Microsoft Power Apps'] },
  { title: 'Tech and Tool Stack', color: 'yellow', items: ['React', 'HTML5', 'CCS3', 'PHP', 'Java', 'Next.js', 'Vite'] }
]
function App() {
  ...
  const showInterest = index => {
    if (index === activeInterest) return
    interestHasBeenUsed.current = true
    setInterestDirection(index > activeInterest ? 'next' : 'previous')
    setActiveInterest(index)
  }
  useEffect(() => {
    if (interestHasBeenUsed.current) interestButtonRefs.current[activeInterest]?.focus()
  }, [activeInterest])
  const handleInterestKeyDown = event => {
    if (event.key !== 'ArrowRight' && event.key !== 'ArrowLeft') return
    event.preventDefault()
    const offset = event.key === 'ArrowRight' ? 1 : -1
    showInterest((activeInterest + offset + interestCards.length) % interestCards.length)
  }
  const currentInterest = interestCards[activeInterest]
  const previousInterest = interestCards[(activeInterest + interestCards.length - 1) % interestCards.length]
  const nextInterest = interestCards[(activeInterest + 1) % interestCards.length]
  return <div className="site-shell"><header className="nav-wrap"><a href="#home" className="brand" onClick={goHome}>ARMANI MAGNIFICO</a><nav className={menuOpen ? 'nav-links is-open' : 'nav-links'}>{navItems.map(([label, id]) => <a onClick={closeMenu} href={`#${id}`} key={id}>{label}</a>)}</nav><div className="header-controls"><button className={dark ? 'theme-switch is-dark' : 'theme-switch is-light'} onClick={() => setDark(value => !value)} aria-label={`Switch to ${dark ? 'light' : 'dark'} mode`} aria-pressed={dark}><span className="theme-switch-icon" aria-hidden="true"><Icon name={dark ? 'moon' : 'sun'} size={16}/></span><span className="theme-switch-knob" aria-hidden="true"/></button><button className="menu-button" onClick={() => setMenuOpen(value => !value)} aria-label="Toggle navigation"><Icon name={menuOpen ? 'close' : 'menu'}/></button></div></header><main>
    <section className="hero" id="home"><div className="hero-content"><p className="hero-greeting">Hello, my name is</p><h1>ARMANI MAGNIFICO</h1><p className="hero-copy">System Design &amp; Software Engineering | Frontend Developer |<br/>Quality Assurance Specialist | 4th year IT Student</p><div className="hero-actions"><a className="social-button social-linkedin" href="https://www.linkedin.com/in/armanimagnifico" target="_blank" rel="noreferrer">LinkedIn <span aria-hidden="true">&gt;</span></a><a className="social-button social-gmail" href="mailto:armanimagnifico4195@gmail.com">Gmail <span aria-hidden="true">&gt;</span></a><a className="social-button social-github" href="https://github.com/armani19" target="_blank" rel="noreferrer">GitHub <span aria-hidden="true">&gt;</span></a></div></div><div className="portrait-orb"><img src={dark ? portraitDark : portrait} alt="Armani Magnifico"/></div></section>
    <section className="about-section" id="about"><div className="about-intro"><div className="about-copy"><h2>About me</h2><p>I&apos;m a fourth-year Information Technology student at FEU Institute of Technology specializing in Web and Mobile Application Development. I enjoy turning ideas and real-world problems into digital solutions that are intuitive, functional, and meaningful to the people who use them.</p><h2>What interests me</h2><p>I&apos;m particularly interested in system design, software engineering, frontend development, and quality assurance. I value understanding how a system works as a whole. From identifying user needs and designing workflows to development, testing, and continuous improvement.</p><h2>Beyond Programming</h2><p>Outside of programming, my experience in student leadership and project coordination has taught me the importance of accountability, collaboration, and being willing to take initiative when needed. I approach every project as an opportunity to learn, improve my craft, and become a more well-rounded IT professional.</p></div></div><div className="about-details"><div className="interest-carousel" onKeyDown={handleInterestKeyDown} aria-label="Interests"><div className="interest-stage" aria-live="polite"><div className={`interest-layer interest-layer--previous interest-card--${previousInterest.color}`} aria-hidden="true"/><div className={`interest-layer interest-layer--next interest-card--${nextInterest.color}`} aria-hidden="true"/><article key={`${activeInterest}-${interestDirection}`} className={`interest-card interest-card--${currentInterest.color} interest-card--${interestDirection}`}><h3>{currentInterest.title}</h3><div className="pill-row">{currentInterest.items.map(item => <span key={item}>{item}</span>)}</div></article></div><div className="interest-nav" role="tablist" aria-label="Select an interest card">{interestCards.map((card, index) => <button key={card.title} ref={element => { interestButtonRefs.current[index] = element }} type="button" role="tab" tabIndex={activeInterest === index ? 0 : -1} aria-label={`Show ${card.title}`} aria-selected={activeInterest === index} aria-current={activeInterest === index ? 'true' : undefined} className={activeInterest === index ? 'is-active' : ''} onClick={() => showInterest(index)} />)}</div></div></div><div className="about-meta"><div><h3>Based in</h3><p>Metro Manila, Philippines</p></div><div><h3>Education</h3><p>BS Information Technology Specializing in Web and Mobile Application<br/>FEU Institute of Technology | Expected 2027</p></div><div><h3>Currently</h3><p>Open to Internship Opportunities<br/>2 Terms | 520 hrs. each term<br/>December 3, 2026 - March 31, 2027 &amp; April 1, 2027 - July 31, 2027</p></div></div></section>
