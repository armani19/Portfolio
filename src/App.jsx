...
function App() {
  ...
  return <div className="site-shell"><header className="nav-wrap"><a href="#home" className="brand" onClick={goHome}>ARMANI MAGNIFICO</a><nav className={menuOpen ? 'nav-links is-open' : 'nav-links'}>{navItems.map(([label, id]) => <a onClick={closeMenu} href={`#${id}`} key={id}>{label}</a>)}</nav><div className="header-controls"><button className="theme-switch" onClick={() => setDark(value => !value)} aria-label={`Switch to ${dark ? 'light' : 'dark'} mode`} aria-pressed={dark}><Icon name={dark ? 'sun' : 'moon'} size={16}/><span>{dark ? 'Light' : 'Dark'}</span></button><button className="menu-button" onClick={() => setMenuOpen(value => !value)} aria-label="Toggle navigation"><Icon name={menuOpen ? 'close' : 'menu'}/></button></div></header><main>
